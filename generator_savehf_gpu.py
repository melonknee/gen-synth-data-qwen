...
def process_flickr30k_images_fixed(image_paths, output_file="qwen_captions_fixed.json", batch_size=1):
    ...
    for i, img_path in enumerate(tqdm(image_paths, desc="Generating captions")):
        try:
            ...
            if (i + 1) % 5 == 0:
                ...
                upload_to_hf(f"temp_{output_file}", "melonknee/qwenmade_flickr30k_captions", path_in_repo=f"temp_{output_file}")
    ...
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    upload_to_hf(output_file, repo_id="melonknee/qwenmade_flickr30k_captions", path_in_repo=output_file)
...
def upload_to_hf(local_path, repo_id, repo_type="dataset", path_in_repo=None, token=HF_TOKEN):
    """
    ...
    upload_file(
        ...
        repo_id=repo_id,
        repo_type=repo_type,
        token=token,  # Optional if you've logged in
